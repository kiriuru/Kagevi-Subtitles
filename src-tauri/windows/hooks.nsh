; Kagevi Subtitles NSIS hooks.
; Runtime folders user-data/ and logs/ are created by the app (and POSTINSTALL),
; never wiped on upgrade/uninstall.

!macro VoiceSubRemoveShippedBinTree rootRel
  ; Fully replaced by bundle.resources on every install — wipe first so Vite
  ; content-hash orphans and Nuitka *.build intermediates cannot survive upgrades.
  RMDir /r "$INSTDIR\${rootRel}\dashboard"
  RMDir /r "$INSTDIR\${rootRel}\worker"
  RMDir /r "$INSTDIR\${rootRel}\tts"
  RMDir /r "$INSTDIR\${rootRel}\local-asr"
  RMDir /r "$INSTDIR\${rootRel}\overlay"
  RMDir /r "$INSTDIR\${rootRel}\fonts"
  RMDir /r "$INSTDIR\${rootRel}\modules"
!macroend
...
!macro NSIS_HOOK_PREINSTALL
  ...
  ; TTS sidecar may still hold file locks after a crash or update race.
  nsExec::ExecToLog 'taskkill /F /IM google_tts_fetch.exe /T'
  Sleep 200

  !insertmacro VoiceSubRemoveShippedBinTree "bin"
  ; Some layouts historically used resources\bin — clear both.
  !insertmacro VoiceSubRemoveShippedBinTree "resources\bin"
!macroend
