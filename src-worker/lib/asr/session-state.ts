...
export function createBrowserAsrStateSeed(existing?: Partial<BrowserAsrState>): BrowserAsrState {
  ...
  return {
    ...
    overlapPrestartTimer: null,
    overlapPrestartTimerArmed: false,
    overlapBuddyArmAttempts: 0,
    overlapSoftRearmEmptyCount: Number(seed.overlapSoftRearmEmptyCount || 0),
    lastOverlapSoftRearmAtMs: Number(seed.lastOverlapSoftRearmAtMs || 0),
    overlapSafeRestartInProgress: Boolean(seed.overlapSafeRestartInProgress),
    overlapBuddyPrestartOkCount: 0,
    overlapBuddyPrestartFailCount: 0,
    overlapBuddyOnstartCount: 0,
    overlapBuddyOnendCount: 0,
    ...
    configuredLanguage: seed.configuredLanguage || "ru-RU",
    sourceLang: seed.sourceLang || "ru",
    socket: seed.socket ?? null,
    forceFinalizeTimer: seed.forceFinalizeTimer ?? null,
    currentPartial: seed.currentPartial || "",
    finalCount: Number(seed.finalCount || 0),
    missingFinalCount: Number(seed.missingFinalCount || 0),
    forcedCount: Number(seed.forcedCount || 0),
    appSendCount: Number(seed.appSendCount || 0),
    ...
    micHealthUpdatedAt: Number(seed.micHealthUpdatedAt || 0),
    settingsSavePromise: seed.settingsSavePromise || Promise.resolve(),
  };
}

export function initializeBrowserAsrState(target: BrowserAsrState, existing?: Partial<BrowserAsrState>): void {
  Object.assign(target, createBrowserAsrStateSeed(existing));
}

export function applyInstanceDefaults(manager: Record<string, unknown> & { restartDelayByReasonMs: Record<string, number> }): void {
  const defaults = INSTANCE_DEFAULTS;
  for (const key of Object.keys(defaults) as (keyof typeof INSTANCE_DEFAULTS)[]) {
    (manager as unknown as Record<string, unknown>)[key] = defaults[key];
  }
  manager.restartDelayByReasonMs = { ...RESTART_DELAY_BY_REASON_MS };
}
