...
pub const PROJECT_VERSION: &str = "0.6.5";
...
fn parse_semver(value: &str) -> Option<SemVer> {
    ...
    let build = parts.next().and_then(|part| part.parse().ok()).unwrap_or(0);
    Some(SemVer {
        major,
        minor,
        patch,
    ...
    fn github_releases_pick_highest_0_5_x_and_trigger_update() {
        let releases = json!([
            ...
            },
            {
                ...
                "draft": false,
                "prerelease": false,
                "html_url": release_url_for(DEFAULT_GITHUB_REPO, "0.7.0")
            }
        ]);
        let (latest, _, url) = extract_latest_github_release(&releases, "stable");
        assert_eq!(latest.as_deref(), Some("0.7.0"));
        assert_eq!(
            url.as_deref(),
            Some(release_url_for(DEFAULT_GITHUB_REPO, "0.7.0").as_str())
        );
        assert!(is_remote_version_newer(
            PROJECT_VERSION,
            latest.as_deref().unwrap()
        ));
        ...
        assert_eq!(
            ...
            release_url_for(DEFAULT_GITHUB_REPO, "0.7.0")
        );
    }
