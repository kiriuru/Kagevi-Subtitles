...
pub fn collect_resource_telemetry() -> ResourceTelemetry {
    ...
    for pid in enum_process_ids() {
        ...
        if let Some(existing) = watched
            .iter_mut()
            .find(|entry| names_match(&entry.name, &snapshot.name))
        {
...
#[cfg(windows)]
unsafe fn read_process_snapshot(
    ...
    unsafe {
        ...
        use windows::Win32::System::Threading::{
            GetProcessHandleCount, PROCESS_NAME_WIN32, QueryFullProcessImageNameW,
        };
