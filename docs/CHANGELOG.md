...
## [Unreleased](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.7.0...HEAD)

## [0.7.0](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.6.5...v0.7.0) - 2026-09-02

### Added

- Модуль **VRChat Chatbox OSC** (`/vrchat`) — финалы пайплайна в Chatbox. Настроил → **Включить вывод** → окно можно закрыть. **KAT нет.** OSC `127.0.0.1:9000` / `9001`; лимит **144** символа / **9** строк; что отправлять — как OBS CC; пауза по MuteSelf / AFK; автоочистка Chatbox (по умолчанию **5 с**). **Тест соединения** / **Отправить тест**. Config: `user-data/modules/vrchat/config.toml`.
- Модуль **SteamVR OpenVR HUD** (`/vr-overlay`) — субтитры только для носителя, отдельно от Chatbox VRChat. Привязка к HMD, контроллеру (запястье ~12 см / ладонь / сверху) или миру; поза запоминается по origin; скругление панели (по умолчанию **16**). Что показывать — исходник и включённые линии перевода. Опциональная панель **Twitch chat** (портрет **480×900**, по умолчанию левый контроллер): бейджи, цвет ников, `display (login)`; саб / рейд / чир только у Broadcaster. **Запустить/Остановить SteamVR** из модуля. Включил → окно можно закрыть. Только PCVR. Config: `user-data/modules/vr-overlay/config.toml`.
- Модуль **Twitch** (`/twitch`) выделен из TTS: IRC, EventSub (фоллоу / саб / гифт / рейд / чир), OAuth стримера, фильтры, chat и event TTS независимо от озвучки субтитров. Хватает Broadcaster-токена; доп. каналы и токен бота необязательны (на доп. каналах — только чат). Пересылка сырого чата в SteamVR HUD — `forward_to_vr_overlay` (нужны оба тумблера). Включил → окно можно закрыть; автоподключение при старте. Config: `user-data/modules/twitch/config.toml`. HTTP `/api/twitch/*`.
- Субтитры: скорость прокрутки overflow (`overlay.scroll_speed_px_per_sec`, по умолчанию **48**, **12–120**). После обновления перезагрузите Browser Source.
- Настройки: **Сбросить к заводским** — профиль `default` и конфиги TTS / Twitch / Local ASR / VRChat / SteamVR HUD. Профили сохраняют тот же полный снимок; загрузка применяется сразу.
- Модули: компактные квадратные карточки (иконка / статус / Открыть) + «?»; логотипы NVIDIA (Local ASR / Parakeet), VRChat и SteamVR.

### Changed

- Замена слов: маска `fuck`→`f**k`, `whore`→`w***e` (первая и последняя буква); формы с `*` не переписываются.
- С **0.7.0** лицензия приложения: Copyright © 2026 Kiriuru. Все права защищены. См. [LICENSE](../LICENSE). Релизы **по 0.6.5** остаются под MIT.
- Статус-бар Эфира закреплён на **всех** вкладках: полный KPI на Эфире, сжатая полоска на остальных.
- «Вписывать субтитры в окно OBS» → **«Прокручивание субтитров»**.
- Тема UI: галерея пресетов (применение **только по клику**), пресет **GTA 6**. Длинные подсказки — за кнопкой **?**.
- Google Web (`google_web`): шлюз `translate-pa.googleapis.com/v1/translateHtml` вместо gtx (`429`/`sorry`). Burst 429 смягчён интервалом по провайдеру; общий **Максимум параллельных задач** — главный лимит.
- Модуль **TTS** (`/tts`) — только озвучка субтитров (нужны Live **Старт** и Enable). Закрытие окна не глушит речь. Twitch OAuth redirect по-прежнему `/tts`.

### Fixed

- Главное окно сразу открывается с bootstrap на `/live`. Leftover `running` после kill — **info**, не WARN, если PID уже мёртв.
- Закрытие окна модуля уничтожает WebView2; сервис без окна живёт только пока модуль **включён**.
- Установщик не копирует dev-остатки из `bin/` (скрипты TTS, распакованные Google Fonts, zip-метаданные). Upgrade сначала чистит `bin/modules` и `bin/fonts`.

## [0.6.5](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.6.4...v0.6.5) - 2026-08-20
...
## [0.6.4](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.6.3...v0.6.4) - 2026-08-08
...
## [0.6.3](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.6.2...v0.6.3) - 2026-08-05
...
- NSIS upgrade: перед копированием ресурсов сносятся shipped-деревья `$INSTDIR\bin\{dashboard,worker,tts,local-asr,vrchat,vr-overlay,overlay,fonts,modules}` (и то же под `resources\bin`), чтобы сиротские Vite content-hash файлы и прочий мусор прошлых сборок не копились при обновлении; `user-data/` и `logs/` не трогаются.
...
## [0.6.2](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.6.1...v0.6.2) - 2026-08-02
...
## [0.6.1](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.6.0...v0.6.1) - 2026-07-30
...
## [0.6.0](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.5.5...v0.6.0) - 2026-07-18
...
## [0.5.5](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.5.4...v0.5.5) - 2026-06-26
...
## [0.5.4](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.5.3...v0.5.4) - 2026-06-21
...
## [0.5.3](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.5.2...v0.5.3) - 2026-06-17
...
## [0.5.2](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.5.1...v0.5.2) - 2026-06-14
...
## [0.5.1](https://github.com/kiriuru/Kagevi-Subtitles/compare/v0.5.0...v0.5.1) - 2026-06-13
...
## [0.5.0](https://github.com/kiriuru/Kagevi-Subtitles/releases/tag/v0.5.0) - 2026-06-10
...
## [0.4.4](https://github.com/kiriuru/stream_sub_translator/compare/v0.4.3...v0.4.4) - 2026-05-31
...
## [0.4.3](https://github.com/kiriuru/stream_sub_translator/compare/v0.4.2...v0.4.3) - 2026-05-27
...
## [0.4.2](https://github.com/kiriuru/stream_sub_translator/compare/v0.4.1...v0.4.2) - 2026-05-25
...
## [0.4.1](https://github.com/kiriuru/stream_sub_translator/compare/v0.4.0...v0.4.1) - 2026-05-20
...
## [0.4.0](https://github.com/kiriuru/stream_sub_translator/compare/v0.3.2...v0.4.0) - 2026-05-16
...
## [0.3.2](https://github.com/kiriuru/stream_sub_translator/compare/v0.3.1...v0.3.2) - 2026-05-14

### Fixed

- Патчи стабильности SST Desktop после 0.3.1.

## [0.3.1](https://github.com/kiriuru/stream_sub_translator/compare/v0.3.0...v0.3.1) - 2026-05-12

### Fixed

- Follow-up фиксы SST Desktop после модульного релиза 0.3.0.

## [0.3.0](https://github.com/kiriuru/stream_sub_translator/compare/v0.2.9.2...v0.3.0) - 2026-05-08
...
## [0.2.9.2](https://github.com/kiriuru/stream_sub_translator/releases/tag/v0.2.9.2) - 2026-04-30
