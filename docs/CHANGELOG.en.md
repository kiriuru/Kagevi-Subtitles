...
- **VRChat Chatbox OSC** module (`/vrchat`) — pipeline finals to VRChat Chatbox. Configure → **Enable output** → close the window. **No KAT.** OSC `127.0.0.1:9000` / `9001`; limit **144** characters / **9** lines; **What to send** matches OBS CC; optional pause on MuteSelf / AFK; Chatbox auto-clear (default **5 s**). **Test connection** / **Send test**. Config: `user-data/modules/vrchat/config.toml`.
- **SteamVR OpenVR HUD** module (`/vr-overlay`) — wearer-only subtitles, separate from VRChat Chatbox. Attach to HMD, a controller (wrist ~12 cm / palm / above), or world; pose is remembered per origin; panel corner radius (default **16**). **What to show** is source plus enabled translation lines. Optional **Twitch chat** panel (portrait **480×900**, default left controller): badges, nick colors, `display (login)`; sub / raid / cheer only on Broadcaster. **Start/Stop SteamVR** from the module. Enable → close the window. PCVR only. Config: `user-data/modules/vr-overlay/config.toml`.
- **Twitch** module (`/twitch`) split out of TTS: IRC, EventSub (follow / sub / gift / raid / cheer), broadcaster OAuth, filters, chat and event TTS independent of subtitle speech. Broadcaster token is enough; extra channels and a bot token are optional (extra channels stay chat-only). Raw chat forward to SteamVR HUD — `forward_to_vr_overlay` (both toggles required). Enable → close the window; auto-connects on start. Config: `user-data/modules/twitch/config.toml`. HTTP `/api/twitch/*`.
- Subtitles: overflow **scroll speed** (`overlay.scroll_speed_px_per_sec`, default **48**, **12–120**). After updating, reload the Browser Source.
- Settings: **Reset to factory defaults** — `default` profile plus TTS / Twitch / Local ASR / VRChat / SteamVR HUD configs. Profiles save that same full snapshot; load applies immediately.
- Modules: compact square cards (icon / status / Open) plus help; NVIDIA (Local ASR / Parakeet), VRChat, and SteamVR logos.
...
- Google Web (`google_web`) uses `translate-pa.googleapis.com/v1/translateHtml` instead of gtx (`429`/`sorry`). Burst 429 is mitigated with a per-provider interval; **Maximum parallel jobs** is the shared cap.
- **TTS** module (`/tts`) is subtitle speech only (needs Live **Start** and Enable). Closing the window does not mute playback. Twitch OAuth redirect stays `/tts`.

### Fixed

- Main window opens with bootstrap on `/live`. Leftover `running` after a kill is **info**, not WARN, when that PID is already dead.
- Closing a module window destroys its WebView2; the service without a window runs only while the module stays **enabled**.
- Installer no longer copies developer leftovers under `bin/` (TTS scripts, unpacked Google Fonts, zip metadata). Upgrade wipes `bin/modules` and `bin/fonts` first.
...
- NSIS upgrade: before copying resources, wipe shipped `$INSTDIR\bin\{dashboard,worker,tts,local-asr,vrchat,vr-overlay,overlay,fonts,modules}` (and the same under `resources\bin`) so orphaned Vite content-hash files and other leftovers from prior builds cannot accumulate across updates; `user-data/` and `logs/` are never touched.
