...
Руководство пользователя для **Kagevi Subtitles `0.6.2`** — как получить живые субтитры на стриме, что делает каждый экран и как чинить типичные проблемы.
...
Текущая линия: **`0.6.2`** (первый релиз Kagevi — `0.5.0`).
...
1. Запустите `Kagevi Subtitles_0.6.2_x64-setup.exe` (или свежий setup со [страницы релизов](https://github.com/kiriuru/Kagevi-Subtitles/releases)).
...
4. По желанию: **Перевод** — включите перевод, активируйте хотя бы одну линию, выберите провайдера (четыре работают **без API key** — ниже).
...
- [ ] Провайдеру нужен ключ? Добавьте в настройках провайдера. Или возьмите **безключевой**: Google Web, Free Web Translate, Bing Translator (Microsoft Edge может отвечать **HTTP 404**).
...
| **Microsoft Edge Translate** | Анонимный Edge / качество Azure, когда путь жив. Может отвечать **HTTP 404**, если Microsoft блокирует endpoint — переключитесь на Bing / Google Web. |
| **Bing Translator** | Keyless Bing web session (`ttranslatev3`) |

Всего **18** провайдеров (Google API, DeepL, Azure, LibreTranslate, OpenAI-compatible, китайские и др.). Ключи хранятся только в локальном `config.toml`.
